...
#[derive(Clone)]
pub struct TokioLocalShardHandle {
    tokio_rt: Handle,
    shard_id: ShardId,
    tx: tokio::sync::mpsc::Sender<Task<TokioShardCtx>>,
}

impl EventLoopHandle for TokioLocalShardHandle {
...
macro_rules! shard_tokio_main {
    ($name:ident) => {
        pub static $name: ::std::sync::LazyLock<::eventful_rs::tokio_local::TokioLocalShard> =
            ::std::sync::LazyLock::new(|| {
                ::eventful_rs::tokio_local::TokioLocalShard::new(stringify!($name))
            });
        use ::eventful_rs::tokio_local::TokioLocalShard as DefaultShardType;
        use ::eventful_rs::tokio_local::TokioLocalShardHandle as DefaultShardHandleType;
        fn default_shard() -> &'static ::eventful_rs::tokio_local::TokioLocalShard {
            &$name
        }
    };
}
